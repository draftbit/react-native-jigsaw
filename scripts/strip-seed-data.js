...
function replaceIdentifiers(file, identifiers) {
  ...
  file = file.replace(new RegExp(IDENTIFIERS.cloudinaryUrl, "g"), CLOUDINARY_URL)
  return file
}
...
async function parseFileSeedData(file) {
  ...
  if (lineStart !== -1) {
    ...
    const str = lines.slice(0, codeStart).filter(line => !line.includes(IDENTIFIERS.import)).join("\n")
    return str
  }
